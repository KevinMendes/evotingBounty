...
public class EncryptedVerifiableVotePayloadDeserializer extends JsonDeserializer<EncryptedVerifiableVotePayload> {

	@Override
	public EncryptedVerifiableVotePayload deserialize(final JsonParser parser, final DeserializationContext deserializationContext)
			throws IOException {
		...
		final EncryptedVerifiableVote vote = mapper.reader().withAttribute("group", gqGroup)
		...
		return new EncryptedVerifiableVotePayload(gqGroup, vote, node.get("requestId").asText(), signature);
	}
}
