...
/**
 * Web service for handling verification card set data.
 */
@Stateless(name = "ag-VerificationCardSetDataResource")
@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
@Path(VerificationCardSetDataResource.RESOURCE_PATH)
public class VerificationCardSetDataResource {

	static final String SAVE_VERIFICATION_CARD_SET_DATA = "tenant/{tenantId}/electionevent/{electionEventId}/verificationcardset/{verificationCardSetId}/adminboard/{adminBoardId}";

	static final String RESOURCE_PATH = "/vv/verificationcardsetdata";
	...
	static final String VERIFICATION_CARD_SET_DATA_PATH = PROPERTIES.getPropertyValue("VERIFICATION_CARD_SET_DATA_PATH");
	...
	@Inject
	VerificationCardSetDataResource(VoteVerificationAdminClient voteVerificationAdminClient, TrackIdGenerator trackIdGenerator) {
		this.voteVerificationAdminClient = voteVerificationAdminClient;
		this.trackIdGenerator = trackIdGenerator;
	}

	@POST
	@Path(SAVE_VERIFICATION_CARD_SET_DATA)
	...
	public Response saveVerificationCardSetData(
		...
		try (ResponseBody responseBody = RetrofitConsumer.processResponse(voteVerificationAdminClient
				.saveVerificationCardSetData(VERIFICATION_CARD_SET_DATA_PATH, tenantId, electionEventId, verificationCardSetId, adminBoardId,
						xForwardedFor, trackingId, body))) {
			return Response.ok().build();
		} catch (RetrofitException rfE) {
			LOGGER.error("Error trying to save verification card set data.", rfE);
