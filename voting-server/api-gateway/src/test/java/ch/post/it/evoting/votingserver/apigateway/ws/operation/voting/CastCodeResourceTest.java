...
public class CastCodeResourceTest extends JerseyTest {

	public static final String URL_CAST_CODE_CONTEXT_DATA = CastCodeResource.RESOURCE_PATH;
	...
	public void getStatusOfVotingCards() throws IOException {
		...
		reply.addProperty("castCodes", "areHere");
		...
		when(votingWorkflowVotingClient
				.getCastCodeMessage(eq(CastCodeResource.CAST_CODE_PATH), eq(TENANT_ID), eq(ELECTION_EVENT_ID), eq(VOTING_CARD_ID), eq(AUTH_TOKEN),
						eq(X_FORWARDER_VALUE), eq(TRACK_ID))).thenReturn(callMock);

		Response response = target(URL_CAST_CODE_CONTEXT_DATA).resolveTemplate("tenantId", TENANT_ID)
				.resolveTemplate("electionEventId", ELECTION_EVENT_ID).resolveTemplate("version", VERSION)
				.resolveTemplate("votingCardId", VOTING_CARD_ID).request().header(RestApplication.PARAMETER_AUTHENTICATION_TOKEN, AUTH_TOKEN).get();
	...
	protected Application configure() {
		...
		sut = new CastCodeResource(votingWorkflowVotingClient, trackIdGenerator);
