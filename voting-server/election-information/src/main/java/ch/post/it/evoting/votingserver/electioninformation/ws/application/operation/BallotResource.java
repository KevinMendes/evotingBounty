...
/**
 * Web service for handling ballot resource.
 */
@Path("/ballots")
@Stateless
public class BallotResource {
	...
	private static final String RESOURCE_NAME = "ballots";
	...
	/**
	 * Return a ballot given the tenant and the ballot identifiers.
	 *
	 ...
	 * @return Returns the corresponding ballot for the tenantId and ballotId.
	...
	public Response getBallot(
		...
		LOGGER.info("Getting ballot for ballotId: {}, electionEventId: {}, and tenantId: {}.", ballotId, electionEventId, tenantId);

		// validate parameters
		validateParameters(tenantId, electionEventId, ballotId);

		// search ballot
		final Ballot ballot = ballotRepository.findByTenantIdElectionEventIdBallotId(tenantId, electionEventId, ballotId);

		LOGGER.info("Ballot with ballotId: {}, electionEventId: {}, and tenantId: {} found.", ballotId, electionEventId, tenantId);

		// convert to string
		final JsonObject jsonObject = JsonUtils.getJsonObject(ballot.getJson());
		final JsonObjectBuilder jsonObjectBuilder = JsonUtils.jsonObjectToBuilder(jsonObject);
		jsonObjectBuilder.add(JSON_PARAMETER_SIGNATURE, ballot.getSignature());
		final String json = gson.toJson(jsonObjectBuilder.build().toString());

		// return the ballot json
