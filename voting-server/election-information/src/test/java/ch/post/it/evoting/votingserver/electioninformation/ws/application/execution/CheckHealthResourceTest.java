...
import ch.post.it.evoting.votingserver.electioninformation.ws.application.operation.CheckHealthResource;
...
public class CheckHealthResourceTest extends JerseyTest {
	...
	public void whenHealthChecksAreHealthyShouldReturnOK() {
		// given
		final HashMap<HealthCheckValidationType, HealthCheck.HealthCheckResult> healthCheckResults = new HashMap<>();
		healthCheckResults.put(HealthCheckValidationType.DATABASE, HealthCheck.HealthCheckResult.healthy());
		final HealthCheckStatus status = new HealthCheckStatus(healthCheckResults);
	...
	public void testIfServiceRunning() {
		...
		healthCheckResults.put(HealthCheckValidationType.DATABASE, HealthCheck.HealthCheckResult.healthy());
		final HealthCheckStatus status = new HealthCheckStatus(healthCheckResults);
		final List<HealthCheckValidationType> validationsToSkip = Arrays.asList(HealthCheckValidationType.LOGGING_INITIALIZED);
