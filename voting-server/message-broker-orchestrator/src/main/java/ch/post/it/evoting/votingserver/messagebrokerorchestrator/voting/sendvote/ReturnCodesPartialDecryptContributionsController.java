...
public class ReturnCodesPartialDecryptContributionsController {
	...
	public ReturnCodesPartialDecryptContributionsController(
	...
	public List<PartiallyDecryptedEncryptedPCCPayload> getChoiceReturnCodesPartialDecryptContributions(
			@PathVariable
			...
			@RequestBody
			final EncryptedVerifiableVotePayload encryptedVerifiableVotePayload) throws ExecutionException, InterruptedException, TimeoutException {

		final String contextId = String.join("-", Arrays.asList(electionEventId, verificationCardSetId, verificationCardId));

		return broadcastProducerService.sendMessagesAwaitingNotification(contextId, Context.VOTING_RETURN_CODES_PARTIAL_DECRYPT_PCC,
				encryptedVerifiableVotePayload, PARTIAL_DECRYPT_PCC_REQUEST_PATTERN, this::deserializePayload);
	}

	private PartiallyDecryptedEncryptedPCCPayload deserializePayload(final byte[] payload) {
		try {
			return objectMapper.readValue(payload, PartiallyDecryptedEncryptedPCCPayload.class);
		} catch (IOException e) {
